...
export default function Home() {
  ...
  // rounds a value up to a "nice" axis ceiling (1/2/5 × a power of ten) so the y-axis labels look clean
  ...
  const seriesValues = stats?.revenueSeries ? (period === "This week" ? stats.revenueSeries.thisWeek : stats.revenueSeries.lastWeek) : [0, 0, 0, 0, 0, 0, 0];
  // keep the same y-axis scale across both weeks so toggling doesn't jump around
  const maxRevenue = Math.max(1, ...(stats?.revenueSeries ? [...stats.revenueSeries.thisWeek, ...stats.revenueSeries.lastWeek] : [0]));
  const chartMax = niceMax(maxRevenue);
  const chartWidth = 620, chartHeight = 198;
  const points = seriesValues.map((value, i) => {
    const x = (i / (seriesValues.length - 1)) * chartWidth;
    const y = chartHeight - (Math.min(value, chartMax) / chartMax) * chartHeight;
    return [x, y];
  });
  const linePath = points.map(([x, y], i) => `${i === 0 ? "M" : "L"}${x} ${y}`).join(" ");
  const areaPath = `${linePath} L${chartWidth} ${chartHeight} L0 ${chartHeight} Z`;
  const yAxisLabels = [1, 0.8, 0.6, 0.4, 0.2, 0].map((frac) => fmtRevenue(chartMax * frac));
    ...
    <AppShell title="Overview" subtitle="Here's how your AI sales team is performing." action={<button className="primary-button" onClick={() => router.push("/automations")}><span>＋</span> Create automation</button>}>
      ...
      <div className="dashboard-grid">
        <article className="card performance-card">
          ...
          <div className="chart-wrap"><div className="chart-y">{yAxisLabels.map((label) => <span key={label}>{label}</span>)}</div><div className="chart"><div className="gridlines"/><svg className="line-chart" viewBox={`0 0 ${chartWidth} ${chartHeight}`} preserveAspectRatio="none"><defs><linearGradient id="fill" x1="0" y1="0" x2="0" y2="1"><stop stopColor="#8267df" stopOpacity=".26"/><stop offset="1" stopColor="#8267df" stopOpacity="0"/></linearGradient></defs><path d={areaPath} fill="url(#fill)"/><path d={linePath} fill="none" stroke="#7961d5" strokeWidth="3"/></svg><div className="chart-labels"><span>Mon</span><span>Tue</span><span>Wed</span><span>Thu</span><span>Fri</span><span>Sat</span><span>Sun</span></div></div></div>
